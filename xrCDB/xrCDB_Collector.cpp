	...
	void	Collector::calc_adjacency	(xr_vector<u32>& dest)
	{
...

		{
			...
			for ( ; _I != _E; ++_I) {
				...
				if ((*_I).vertex_id0 != (*J).vertex_id0)
					continue;

				if ((*_I).vertex_id1 != (*J).vertex_id1)
					continue;

				dest[(*_I).face_id*3 + (*_I).edge_id]	= (*J).face_id;
				dest[(*J).face_id*3 + (*J).edge_id]	= (*_I).face_id;
			}
		}
#	if 0
