...
inline_ BOOL OBBCollider::TriBoxOverlap()
{
	...
	if(min_>mBoxExtents.x || max_<-mBoxExtents.x) return FALSE;

	FINDMINMAX(v0.y, v1.y, v2.y, min_, max_);
	if(min_>mBoxExtents.y || max_<-mBoxExtents.y) return FALSE;

	FINDMINMAX(v0.z, v1.z, v2.z, min_, max_);
	if(min_>mBoxExtents.z || max_<-mBoxExtents.z) return FALSE;
#endif
	// 2) Test if the box intersects the plane of the triangle
	// compute plane equation of triangle: normal*x+d=0
	// ### could be precomputed since we use the same leaf triangle several times
...
inline_ BOOL AABBCollider::TriBoxOverlap()
{
	...
	float _min_,_max_;
	// Find min, max of the triangle in x-direction, and test for overlap in X
	FINDMINMAX(v0.x, v1.x, v2.x, _min_, _max_);
	if(_min_>extents.x || _max_<-extents.x) return FALSE;
	...
	FINDMINMAX(v0.y, v1.y, v2.y, _min_, _max_);
	if(_min_>extents.y || _max_<-extents.y) return FALSE;
	...
	FINDMINMAX(v0.z, v1.z, v2.z, _min_, _max_);
	if(_min_>extents.z || _max_<-extents.z) return FALSE;
#endif
	// 2) Test if the box intersects the plane of the triangle
	// compute plane equation of triangle: normal*x+d=0
	// ### could be precomputed since we use the same leaf triangle several times
