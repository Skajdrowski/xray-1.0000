		...
		case IRender_interface::SM_NORMAL:
			{
				string64			t_stemp;
				string_path			buf;
				sprintf				(buf,"ss_%s_%s_(%s).jpg",Core.UserName,timestamp(t_stemp),(g_pGameLevel)?g_pGameLevel->name().c_str():"mainmenu");
				ID3DXBuffer*		saved	= 0;
				...
				if (strstr(Core.Params,"-ss_tga"))	{ // hq
					sprintf				(buf,"ssq_%s_%s_(%s).tga",Core.UserName,timestamp(t_stemp),(g_pGameLevel)?g_pGameLevel->name().c_str():"mainmenu");
					saved	= 0;
