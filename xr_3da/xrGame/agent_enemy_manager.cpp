...
void CAgentEnemyManager::assign_enemies			()
{
	for (;;) {
		...
		for (u32 _i=0, n = m_enemies.size() - 1; _i<n; ++_i)
			if (m_enemies[_i + 1] < m_enemies[_i])
				std::swap				(m_enemies[_i],m_enemies[_i + 1]);
			else
				break;
	}
}
		...
		for (int i=0; i<2; ++i) {
			...
			for ( ; I != E; ++I) {
				...
				for ( ; J; J &= J - 1) {
					...
					CAgentMemberManager::iterator	_i = object().member().member(K);
					float							distance_sqr = (*_i)->object().Position().distance_to_sqr((*I).m_object->Position());
