...
void CEntityCondition::UpdateHealth()
{
	...
	if (GameID() == GAME_SINGLE && (GetHealth() + m_fDeltaHealth <= 0.005f)) // When health gets really low, teleport to spawn.
	{
		CActor* pActor = smart_cast<CActor*>(m_object);
		if (pActor)
		{
			pActor->TeleportActor();
			m_fDeltaHealth = 0; // Prevent from lowering health, after teleporting
		}
	}
}
