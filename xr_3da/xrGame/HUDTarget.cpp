...
void CHUDTarget::Render()
{
	...
	if (psHUD_Flags.test(HUD_INFO)){ 
		if (RQ.O){
			CEntityAlive*	EAlive		= smart_cast<CEntityAlive*>	(RQ.O);
			...
			if (IsGameTypeSingle())
			{
				...
				if (EAlive && EAlive->g_Alive() && !EAlive->cast_base_monster())
				{
//.					CInventoryOwner* our_inv_owner		= smart_cast<CInventoryOwner*>(pCurEnt);
					CInventoryOwner* others_inv_owner	= smart_cast<CInventoryOwner*>(EAlive);
			...
			{
				if (EAlive && (EAlive->GetfHealth()>0))
				{
					if (pCurEnt && GameID() == GAME_SINGLE){	
						if (GameID() == GAME_DEATHMATCH)			C = C_ON_ENEMY;
						else{	
							if (EAlive->g_Team() != pCurEnt->g_Team())	C = C_ON_ENEMY;
							else									C = C_ON_FRIEND;
						};
