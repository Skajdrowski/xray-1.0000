...
void CRestrictions::Dump() const
{
	...
	for(u32 i=0; i<_RANK_COUNT+1; ++i)
	{
		...
		rank_rest_vec::const_iterator rankit		= v.begin();
		rank_rest_vec::const_iterator rankit_e		= v.end();
		if(i<_RANK_COUNT)
			Msg("---	for rank %d  ---count=[%d]", i, v.size());
		else
			Msg("---	base restrictions ---count=[%d]", v.size());

		for(;rankit!=rankit_e;++rankit)
		{
			Msg("	[%s]:[%d]", (*rankit).first.c_str(), (*rankit).second);
		}
		Msg("-----------------------------------------");
	}
}
