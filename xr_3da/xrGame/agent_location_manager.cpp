...
//			if ((*I)->object().Position().distance_to_sqr(location->position()) <= object->Position().distance_to_sqr(location->position()))
	...
	if (use_enemy_info) {
		CAgentEnemyManager::ENEMIES::const_iterator	enemiesI = this->object().enemy().enemies().begin();
		CAgentEnemyManager::ENEMIES::const_iterator	enemiesE = this->object().enemy().enemies().end();
		for ( ; enemiesI != enemiesE; ++enemiesI)
			if ((*enemiesI).m_enemy_position.distance_to_sqr(location->position()) < _sqr(MIN_SUITABLE_ENEMY_DISTANCE))
				return				(false);
	}
