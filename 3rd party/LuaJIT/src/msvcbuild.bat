...
@set DEBUGCFLAGS=
@set LJCOMPILE=cl /nologo /c /O2 /W3 /D_CRT_SECURE_NO_DEPRECATE /D_SECURE_SCL=0 /D_CRT_STDIO_INLINE=__declspec(dllexport)__inline /std:c++20
...
%LJCOMPILE% luajit.c
@if errorlevel 1 goto :BAD
%LJLINK% /out:lua53.exe luajit.obj %LJLIBNAME%
@if errorlevel 1 goto :BAD
if exist lua53.exe.manifest^
  %LJMT% -manifest lua53.exe.manifest -outputresource:lua53.exe
